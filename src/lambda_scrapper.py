...
class SearchResult(object):
    ...
    def get_carrier_name_by_id(self, carrier_id: str) -> object:
        carrier = next(filter(lambda c: c["Id"] == carrier_id, self.carriers))
        return carrier["Name"]

    def get_agent_name_by_id(self, agent_id: str) -> object:
        agent = next(filter(lambda a: a["Id"] == agent_id, self.agents))
        return agent["Name"]

    def get_leg_by_outbound_id(self, outbound_leg_id: str) -> Dict[str, Any]:
    ...
    def get_iata_code_by_station_id(self, station_id: str) -> object:
        station = next(filter(lambda s: s["Id"] == station_id, self.places))
        return station["Code"]


def create_session_for_flight_direction(departure_city: str, arrival_city: str, outbound_date: str) -> object:
    ...
    payload = (
        f"country={country}&currency={currency}&locale={locale}&originPlace={origin_place_id}"
        f"&destinationPlace={destination_place_id}&outboundDate={outbound_date}&adults={adults}"
    )
    headers = {
    ...
    response = perform_http_call("POST", url, headers, payload, None)
    location = response.headers["Location"]
    return location.rsplit("/", 1)[-1]


def poll_for_results(session_key: object) -> SearchResult:
    ...
    response = perform_http_call("GET", url, headers, None, querystring).json()
    return SearchResult(
...
def perform_http_call(http_method: str, url: str, headers: Dict[str, str], body: object, params: object) -> Response:
    response = requests.request(http_method, url, data=body, headers=headers, params=params)
    response.raise_for_status()
    return response


def process_flight_direction(
    departure_city: str, arrival_city: str, outbound_date: str, global_result: List[Any], current_date_time: datetime,
) -> None:
    session_key = create_session_for_flight_direction(departure_city, arrival_city, outbound_date)
    result = poll_for_results(session_key)
...
def persist_to_s3_bucket(global_result: List[Any], current_date_time: datetime) -> None:
    logger.info(
...
def flights_collector(event: Dict[Any, Any], context: object) -> object:
    ...
    for idx, city_combination_with_date in enumerate(islice(city_combinations_with_dates, start, finish)):
        ...
        logger.info(
            f"Processing [{idx}] direction. Departure station [{city_combination[0]}] arrival station "
            f"[{city_combination[1]}] departure date [{city_combination_with_date[1]}]"
        )

        try:
            process_flight_direction(
                city_combination[0],
                city_combination[1],
                city_combination_with_date[1].strftime("%Y-%m-%d"),
                global_result,
                current_date_time,
            )
        except requests.exceptions.HTTPError:
            logger.exception("Http error occured during request to SkyScanner API. Persisting collected results")
            persist_to_s3_bucket(global_result, current_date_time)

            processed_flights_count += len(global_result)
            logger.info(
                f"Processed [{processed_flights_count}] flight combinations out of "
                f"[{len(city_combinations_with_dates)}] total"
            )
            return {"processed_flights_count": processed_flights_count, "continue": True}
    ...
    return {"processed_flights_count": processed_flights_count, "continue": finish < len(city_combinations_with_dates)}
